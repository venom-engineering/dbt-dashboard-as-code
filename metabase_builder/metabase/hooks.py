...
class MetabaseHook:
    ...
    def get_databases(self) -> Dict[int, Database]:
        ...
        databases = [
            Database(**database) for database in self.get(endpoint="database")["data"]
        ]
    ...
    def get_tables(self) -> Dict[int, Table]:
        ...
        tables = [
            table
            for table in self.get(endpoint="table")
            # if table['db_id'] == 2
        ]
    ...
    def get_database_fields(self, database_id) -> Dict[int, Field]:
        fields = [
            Field(**field)
            for field in self.get(endpoint=f"database/{database_id}/fields")
        ]
